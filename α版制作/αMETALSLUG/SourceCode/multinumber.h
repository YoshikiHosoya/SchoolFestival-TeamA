...
public:
	...
	CMultiNumber();
	CMultiNumber(OBJ_TYPE obj) : CScene(obj) {};
	~CMultiNumber();
	HRESULT Init();													//初期化
	void Uninit();													//終了
	void Update();													//更新
	...
	static CMultiNumber *Create(D3DXVECTOR3 pos, D3DXVECTOR3 onesize, int nValue, int nDigits, CScene::OBJ_TYPE objtype);	//生成処理
