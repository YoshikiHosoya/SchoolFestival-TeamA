...
{
	//前回の入力情報を保存
	m_pState_List.emplace_back(m_pState);

	//保存する限界まで来た時
	if (m_pState_List.size() > MAX_SAVE_KEYSTATE)
	{
		//一番古い情報を消す
		m_pState_List.erase(m_pState_List.begin());
	}
...
{
	if (Frame >= m_pState_List.size())
	{
		Frame = m_pState_List.size();
	}

	for (size_t nCnt = m_pState_List.size() - Frame; nCnt < m_pState_List.size(); nCnt++)
	{
		//1F前は入力されておらず今入力されている時
		if (m_pState_List[nCnt].Gamepad.wButtons & Button && !(m_pState_List[nCnt - 1].Gamepad.wButtons & Button))
		{
			//true
			return true;
		}
	...
	{
		if (m_pState.Gamepad.bLeftTrigger > XINPUT_GAMEPAD_TRIGGER_THRESHOLD && (m_pState_List.end() - 1)->Gamepad.bLeftTrigger <= XINPUT_GAMEPAD_TRIGGER_THRESHOLD)
		{
			// 左トリガーが押された
			return true;
		}
	...
	{
		if (m_pState.Gamepad.bRightTrigger > XINPUT_GAMEPAD_TRIGGER_THRESHOLD && (m_pState_List.end() - 1)->Gamepad.bRightTrigger <= XINPUT_GAMEPAD_TRIGGER_THRESHOLD)
		{
			// 左トリガーが押された
			return true;
		}
