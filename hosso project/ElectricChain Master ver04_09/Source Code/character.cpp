...
//モデル設定
//------------------------------------------------------------------------------
void CCharacter::SetParameter(CHARACTER_TYPE chara, PARAM_TYPE Paramtype, std::shared_ptr<CCharacter> MyPtr)
{
	//モデルキャラクター生成
	m_pModelCharacter = CModelCharacter::Create(chara);
...
void CCharacter::Collision()
{
	...
	if (m_bJump == true && bJump == false && m_modeltype == CHARACTER_PLAYER)
	{
		//音再生
