...
void CBaseMode::DebugCommand()
{
	...
	CDebugProc::Print(CDebugProc::PLACE_RIGHT, "テンキー [8] : デバッグルーム\n");
	...
	{
		CManager::GetRenderer()->GetFade()->SetModeFade(CManager::MODE_DEBUG_ROOM);
	}
	//モーションビューワ
...
void CBaseMode::BaseLoad(HWND hWnd)
{
	...
	//シェーダの読み込み
	if (FAILED(CShader::Load()))
	{
		//失敗
		MessageBox(hWnd, "シェーダ読み込み失敗", "CShader", MB_OK | MB_ICONHAND);
	}
}
