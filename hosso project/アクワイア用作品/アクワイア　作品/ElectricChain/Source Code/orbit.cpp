...
void COrbit::Draw()
{
	...
	//テクスチャの設定
	pDevice->SetTexture(0, CTexture::GetTexture(CTexture::TEX_ORBIT_NORMAL));
...
//生成処理
//------------------------------------------------------------------------------
std::shared_ptr<COrbit> COrbit::Create(D3DXMATRIX *pMtx)
{
	//メモリ確保
	std::shared_ptr<COrbit> pOrbit(new COrbit);
