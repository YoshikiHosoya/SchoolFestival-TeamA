...
void CParticle::Draw()
{
	...
	//テクスチャの設定
	pDevice->SetTexture(0, CTexture::GetTexture(CTexture::TEX_EFFECT_PARTICLE));
...
{
	//メモリ確保
	std::shared_ptr<CParticle> pParticle(new CParticle);
	...
	if (pParticle)
	{
		...
		pParticle->SetObjType(OBJTYPE_EFFECT_PARTICLE);
		pParticle->AddSharedList(pParticle);
	}
...
//頂点の更新
//------------------------------------------------------------------------------
void CParticle::UpdateAllParticle()
{
	//IDリセット
	ResetVertexID();
	...
	CScene::GetSceneList(CScene::OBJTYPE_EFFECT_PARTICLE, pSceneList);
	//nullcheck
	if (!pSceneList.empty())
	{
		//要素分
		for (size_t nCnt = 0; nCnt < pSceneList.size(); nCnt++)
		{
			...
			CParticle *pParticle = ((CParticle*)pSceneList[nCnt].get());

			//nullcheck
			if (pParticle)
			{
				//頂点の更新
				pParticle->UpdateVartex();
			}
		}
	}
	ResetVertexID();
