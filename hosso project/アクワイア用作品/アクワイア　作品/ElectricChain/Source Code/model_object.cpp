...
//静的メンバ変数の初期化
//------------------------------------------------------------------------------
std::shared_ptr<CModelInfo> CModel_Object::pModelInfo = std::make_shared<CModelInfo>();
...
HRESULT CModel_Object::Init()
{
	...
	pModelInfo->aName = "data/MODEL/Map/Wall.x";

	//モデル情報読み込み
	CHossoLibrary::CreateModelInfo(pModelInfo.get());

	//モデル設定
	CSceneX::BindModelInfo(pModelInfo);
...
void CModel_Object::Create(D3DXVECTOR3 pos,D3DXVECTOR3 rot)
{
	...
	//コリジョン生成
	pModelObject->m_pCollision = CCollision::Create(pModelObject->GetMtx(), CCollision::COLLISION_OBJECT);

	//マトリックス計算
	CHossoLibrary::CalcMatrix(pModelObject->GetMtx(), ZeroVector3, pModelObject->GetRot());
